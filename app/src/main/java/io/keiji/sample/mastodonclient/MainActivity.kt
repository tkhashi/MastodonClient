       ...
       val binding: ActivityMainBinding =
        ...
        binding.bottomNavigation.setOnNavigationItemSelectedListener {
            val fragment = when(it.itemId){
                R.id.menu_home  -> TootListFragment.newInstance(TimelineType.HomeTimeline)
                R.id.menu_public -> TootListFragment.newInstance(TimelineType.PublicTimeline)
                else -> null
            }
        ...
        if (savedInstanceState == null){
            val fragment = TootListFragment.newInstance(TimelineType.HomeTimeline)
            supportFragmentManager.beginTransaction()
                .add(
                    R.id.fragment_container,
                    fragment,
